...
int main(void) {
  ...
  while (running) {
    ...
    case TUI_MENU_ADD_TODO: {
      ...
      char status_str[2];
      if (tui_input_text(status_str, count, "Is done? [y/N]: ") ==
          TUI_ERR_EMPTY_INPUT) {
        strcpy(status_str, "n");
      }

      if ((todo_result = todo_add(title, *status_str == 'y', &todos, &count,
                                  &global_id, &capacity)) == TODO_OK) {
    ...
    case TUI_MENU_TOGGLE_STATUS: {
      size_t id = 0;
      if ((tui_result = tui_input_number(&id, "Enter ID to toggle status: ")) !=
          TUI_OK) {
        continue;
      }
    ...
    case TUI_MENU_EDIT_TITLE: {
      size_t id = 0;
      if ((tui_result = tui_input_number(&id, "Enter ID to edit todo: ")) !=
          TUI_OK) {
        continue;
      }
    ...
    case TUI_MENU_DELETE_TODO: {
      size_t id = 0;
      if ((tui_result = tui_input_number(&id, "Enter ID to delete todo: ")) !=
          TUI_OK) {
        continue;
      }
    ...
    case TUI_MENU_LOAD_TODOS: {
      ...
      if ((todo_result = todo_load(TODO_FILE, &todos, &capacity, &count)) ==
          TODO_OK) {
